...
class SentenceDataStream(object):
    ...
    def __init__(self, qa_file_path, vocab, mode):
        ...
        self.q_idx_matrix_batches = [] # batch matrix consists of padded sequence of indices
        self.a_idx_matrix_batches = []
        self.q_len_denom_batches = [] # batch of 1/length for each sentence in batch, technically matrix: (batch_size, 1)
        self.a_len_denom_batches = []
        self.label_batches = []
    ...
    def _prepare_HITNLP_data(self, qa_file_path):
        ...
        q_denom = cfg.ModelConfig.Q_DENOMINATOR
        a_denom = cfg.ModelConfig.A_DENOMINATOR
        assert q_denom in {'SENTENCE_LEN', 'SET_LEN'}
        assert a_denom in {'SENTENCE_LEN', 'SET_LEN'}
        ...
        for l in f:
            ...
            if self.t_e_mode == 't':
                self.instances.append([q_idx_seq, a_idx_seq, q_len_denom, a_len_denom, label, w_overlap, n_s_overlap, idf_overlap, idf_n_s_overlap])
            else:
                self.instances.append([q_idx_seq, a_idx_seq, q_len_denom, a_len_denom, w_overlap, n_s_overlap, idf_overlap, idf_n_s_overlap]) # in evaluation mode results are generated by model and saved later
        ...
        for (batch_start, batch_end) in self.batch_span:
            ...
            q_len_denom_batch = np.zeros((cur_batch_size, 1),
                                   dtype = np.float32
                                   )
            a_len_denom_batch = np.zeros((cur_batch_size, 1),
                                   dtype = np.float32
                                   )
            ...
            for i in range(cur_batch_size):
                ...
                q_len_denom_batch[i] = np.array(q_len_lst_to_model[i], dtype=np.float32)
                a_len_denom_batch[i] = np.array(a_len_lst_to_model[i], dtype=np.float32)
            ...
            self.q_len_denom_batches.append(q_len_denom_batch)
            self.a_len_denom_batches.append(a_len_denom_batch)
    ...
    def get_batch(self):
        ...
        for (batch_start, batch_end) in batch_span:
            q_idx_batch_matrix = self.q_idx_matrix_batches[batch_num]  # batch matrix consists of padded sequence of indices
            ...
            if self.t_e_mode == 't':
                yield (q_batch_tensor, a_batch_tensor,
                       self.q_len_denom_batches[batch_num], self.a_len_denom_batches[batch_num],
                       self.label_batches[batch_num], self.add_feat_batches[batch_num])
            else:
                yield (q_batch_tensor, a_batch_tensor,
                       self.q_len_denom_batches[batch_num], self.a_len_denom_batches[batch_num],
                       self.add_feat_batches[batch_num])
            batch_num += 1
